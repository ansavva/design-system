...
Consumer-facing agent skills for `@ansavva/design-system`. Installed by
...
- `node_modules/@ansavva/design-system/README.md` — ships in the tarball, so
  it matches the installed version exactly.
- The workbench at <https://ansavva.github.io/design-system/> — the only
...
`.claude/skills` among them, so `npx skills add ansavva/design-system`
