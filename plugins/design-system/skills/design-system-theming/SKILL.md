...
description: >-
  Consumer. Re-brand @ansavva/design-system without forking it — the
  semantic role names to override, the CSS custom property seam on web, the
  ThemeProvider seam on React Native — colours, and since 0.19.0 corner radii
...
`radii` and `fonts` need **0.19.0 or later**. Before that the provider carried
...
jsdom. The workbench at <https://ansavva.github.io/design-system/> renders
