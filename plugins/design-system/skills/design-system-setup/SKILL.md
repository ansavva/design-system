...
description: >-
  Consumer. Install and wire up @ansavva/design-system for the first time —
  ...
  install returns 401 or 404 for the @ansavva scope, when components render
  completely unstyled on the web, or when dark mode does not follow.
---

# Setting up @ansavva/design-system
...
`react` and `react-dom` are peer dependencies. `@ansavva/tokens` is a
...
```css
@source '../node_modules/@ansavva/design-system';
```
...
it resolves from your own. Same for `@ansavva/tokens`.
...
TypeScript need to be told to include `@ansavva/design-system` — the symptom
...
```bash
cat node_modules/@ansavva/design-system/CHANGELOG.md
```
...
```bash
gh release list --repo ansavva/design-system
```
...
| Symptom | Cause |
| --- | --- |
| `401`/`404` installing `@ansavva/*` | No `NODE_AUTH_TOKEN`, or a token without `read:packages`. A 404 here usually means unauthenticated, not missing. |
