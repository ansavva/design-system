...
|---|---|---|
| `@ansavva/design-system` | `packages/design-system` | web and React Native consumers |
| `@ansavva/tokens` | `packages/tokens` | React Native consumers, and any tool that needs raw token values |
...
No gate catches this. Nothing in `pr.yml` or `publish.yml` looks at how many
bumps share a push, and the only place the loss is visible is the registry
itself, after the fact. It has not happened here — every version bump on `main`
so far arrived in its own push, which is why every tag has a package behind it —
and that is luck about merge timing, not a property of the pipeline.
