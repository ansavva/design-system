# Changelog — `@ansavva/tokens`

Every version that reached the registry, newest first. This file ships **inside
the package**, so it can be read at `node_modules/@ansavva/tokens/CHANGELOG.md`
...
**Widen your range to take this:** `^0.5.x` will not resolve it.
...
- **No token value changed, and nothing this package exports changed.** The
  ...
  renders for `@ansavva/design-system`, so that the named spacing steps stop
...
- **This file, and it now ships inside the package.** `CHANGELOG.md` is listed
  ...
  `node_modules/@ansavva/tokens/CHANGELOG.md` without leaving the consuming
