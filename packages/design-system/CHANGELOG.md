# Changelog — `@ansavva/design-system`

Every version that reached the registry, newest first. This file ships **inside
the package**, so it can be read at `node_modules/@ansavva/design-system/CHANGELOG.md`
...
exactly as it did on 0.20.0.

- **`ThemeProvider`'s `fonts` takes `body` now — and it reaches every control.**
  0.19.0 refused it on the grounds that the native leaves had never set a body
...
- **What `body` reaches**: every text a native leaf renders that is not a
  heading or mono — 49 leaves, the twenty-one 0.20.0 added included. Before
...
[#16](https://github.com/ansavva/design-system/pull/16)
...
- **`ThemeProvider` takes `radii` and `fonts` now — and 0.18.0's entry was
  ...
  straight out of `@ansavva/tokens` into `StyleSheet.create`, which runs once
  at module load and no context can reach. They are read at render time now,
  the same way colours have been since 0.13.0.
...
[#15](https://github.com/ansavva/design-system/pull/15)
...
- **This file, and it now ships inside the package.** `CHANGELOG.md` is listed
  ...
  `node_modules/@ansavva/design-system/CHANGELOG.md` without leaving the
...
- `styles/theme.css` regenerated against `@ansavva/tokens` 0.3.0, which
