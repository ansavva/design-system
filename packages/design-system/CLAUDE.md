...
`@ansavva/design-system` (0.6.x). Human docs: [`README.md`](README.md).
...
- **Three files per component**: `<name>.props.ts` (shared), `<name>.web.tsx`
  ...
  `@ansavva/tokens`). The per-component `index.ts` re-exports the
...
- **The same rule now covers RADII and the type FAMILY, for a different
  ...
  0.19.0. A value baked into `StyleSheet.create` cannot see a React context any
...
- **Every `Text` and `TextInput` a native leaf renders applies the body family
  ...
  platform sans it always was; what changed in 0.21.0 is that a consumer's
...
- **Never declare `react-native` or `@ansavva/tokens` here** — not as
  ...
  consumer's container build once. `@ansavva/tokens` is published now, so it would no
