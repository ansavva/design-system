...
|---|---|---|
| `@ansavva/design-system` | [`packages/design-system`](packages/design-system) | Platform-split components — one props module, a React DOM + Tailwind web leaf, a React Native leaf |
| `@ansavva/tokens` | [`packages/tokens`](packages/tokens) | One `tokens.json` rendered into typed TypeScript, Tailwind custom properties, and plain JSON |
...
```
/plugin marketplace add ansavva/design-system
```

```
/plugin install design-system@ansavva
```
...
```bash
npx skills@latest add ansavva/design-system
```
